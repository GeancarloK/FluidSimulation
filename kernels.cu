	...
	int sizeBlock)
{
	...
	const float xVelExit = (xIndex_1B == -1) ? velFlux * areaFlux : xVel0[xIndex_1B] * xArea[xIndex_1B];

	const float yVelEntry = (y == 0) ? 0.0 : yVel0[index] * yArea[index];
	const float yVelExit = (yIndex_1B == -1) ? 0.0 : yVel0[yIndex_1B] * yArea[yIndex_1B];

	const float zVelEntry = (z == 0) ? 0.0 : zVel0[index] * zArea[index];
	const float zVelExit = (zIndex_1B == -1) ? 0.0 : zVel0[zIndex_1B] * zArea[zIndex_1B];
