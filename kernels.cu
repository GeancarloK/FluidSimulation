	...
	int zThreads)
{
	...
	*/
	...
	if (xA != 0 && x != 0)
	{
		...
		const double m_xm1 = __ldg(&mass[index - 1]);
		const double v_xm1 = __ldg(&volume[index - 1]);
	...
	if (yA != 0 && y != 0)
	{
		...
		const double m_ym1 = __ldg(&mass[index - xThreads]);
		const double v_ym1 = __ldg(&volume[index - xThreads]);
	...
	if (zA != 0 && z != 0)
	{
		...
		const double m_zm1 = __ldg(&mass[index - xyThreads]);
		const double v_zm1 = __ldg(&volume[index - xyThreads]);
