...
std::pair<double, int> generateCubes(Mesh& objectMesh, std::vector<bool>& cubos, std::vector<float>& mass, std::vector<float>& volume, std::vector<float>& areaX, std::vector<float>& areaY, std::vector<float>& areaZ, float beginMass, float volThread)
{
